...
class DeviceContextEvidenceTests(unittest.TestCase):
    ...
    def _step(self, number: int, **state: object) -> dict:
        diagnosis = _diagnose([_cycle(1, 92.0)], [self._started(**state)])
    ...
    def test_the_busiest_application_is_stated_as_a_share_since_boot(self):
        ...
        label, value = next(
            (label, value) for label, value in _facts(step) if label == "Top application"
        )
        self.assertEqual(label, "Top application")
    ...
    def test_an_uneven_session_distribution_is_amber_in_the_fact_table(self):
        step = self._step(
            3,
    ...
    def test_the_distribution_says_whether_the_saturated_dataplane_was_given_more(self):
        ...
        step = diagnosis["steps"][3]
        hypothesis = next(
            item for item in step["hypotheses"] if item["key"] == "chassis_imbalance"
        )

        self.assertIn("200000 active sessions", hypothesis["text"])
        self.assertIn("the imbalance is in what it was given", hypothesis["text"])

    def test_a_line_card_that_is_not_up_is_named_as_a_finding(self):
        hypotheses = self._hypotheses(
            chassis_status={
                "parsed": True,
                "populated_slots": 2,
    ...
    def test_a_chassis_with_every_card_up_names_no_finding(self):
        hypotheses = self._hypotheses(
            chassis_status={
                "parsed": True,
                "populated_slots": 1,
